...
def main(config):
    ...
    # 3D Mesh
    plot_femesh_plotly_2(rotated_mesh, setup.pde['compartments'], 0, f"reference_cylinder_bp{config['fan_bend_point']}_ban{config['fan_bend_angle']}_bax{config['fan_bend_axis']}_r{config['rotation_inx']}_h".replace('.', '_'), f"reference_cylinder_bp{config['fan_bend_point']}_ban{config['fan_bend_angle']}_bax{config['fan_bend_axis']}_r{config['rotation_inx']}_p".replace('.', '_'))
    # Point cloud
    plot_point_cloud_plotly(rotated_mesh['points'], 0, f"reference_cylinder_bp{config['fan_bend_point']}_ban{config['fan_bend_angle']}_bax{config['fan_bend_axis']}_r{config['rotation_inx']}_pc_h".replace('.', '_'), f"reference_cylinder_bp{config['fan_bend_point']}_ban{config['fan_bend_angle']}_bax{config['fan_bend_axis']}_r{config['rotation_inx']}_pc_p".replace('.', '_'))
    
    # Create logging directory
    ops.makedirs(config['log_directory'], exist_ok=True)
    
    # Saving complete reference mesh file
    tch.save(rotated_mesh, f"{config['log_directory']}/reference_cylinder_bp{config['fan_bend_point']}_ban{config['fan_bend_angle']}_bax{config['fan_bend_axis']}_r{config['rotation_inx']}_mesh.pth".replace('.', '_'))
    
    # filename = "1low_res_sphere2_dir/1low_res_sphere2_no_ecs0.5_refinement8_mesh.1"
    filename = config['main_mesh_file']
    femesh_all_2 = read_tetgen(filename)
    
    # Elongate the mesh to make it appear thin for beading
    # femesh_all_2 = ellipsoidal_scale_mesh(femesh_all_2, tch.tensor([1, 1, 5]))
    ...
    tch.save(all_latents_generated[:full_range], f"{config['log_directory']}/all_latents_generated.pth")
    ...
    create_video_from_two_folders(f"{config['experiment_name']}_p", 
                                    ...
                                    f"reference_cylinder_bp{config['fan_bend_point']}_ban{config['fan_bend_angle']}_bax{config['fan_bend_axis']}_r{config['rotation_inx']}_p".replace('.', '_') + "/femesh_plot_iter_0.png", 
                                    f"reference_cylinder_bp{config['fan_bend_point']}_ban{config['fan_bend_angle']}_bax{config['fan_bend_axis']}_r{config['rotation_inx']}_pc_p".replace('.', '_') + "/femesh_plot_iter_0.png", 
