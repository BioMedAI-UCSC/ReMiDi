...
def main(config):
    ...
    def load_model_from_checkpoint(opt, job_id):
        ...
        with open(f"{config['checkpoint_base_path']}/" + str(job_id) + '/infos.json', "r") as outfile:
        ...
        best_checkpoint_filename = ops.path.join(config['checkpoint_base_path'], str(job_id), "last.ckpt")
        print('Loading checkpoint:', best_checkpoint_filename)
    
        # Load the model from the checkpoint
        opt['model'] = model_class.load_from_checkpoint(best_checkpoint_filename, opt=opt).to(opt["device"])
    ...
    opt['evecs'] = tch.from_numpy(np.load(config['path_evecs'])).float().to(opt['device'])[:, :opt['nb_freq']]
    ...
    # 3D Mesh
    plot_femesh_plotly_2(femesh_all_2, setup.pde['compartments'], 0, f"reference_cylinder_{config['deformation_bend']}{config['deformation_twist']}_h".replace('.', '_'), f"reference_cylinder_{config['deformation_bend']}{config['deformation_twist']}_p".replace('.', '_'))
    # Point cloud
    plot_point_cloud_plotly(femesh_all_2['points'], 0, f"reference_cylinder_{config['deformation_bend']}{config['deformation_twist']}_pc_h".replace('.', '_'), f"reference_cylinder_{config['deformation_bend']}{config['deformation_twist']}_pc_p".replace('.', '_'))
    
    # Create logging directory
    ops.makedirs(config['log_directory'], exist_ok=True)
    
    # Saving complete reference mesh file
    tch.save(femesh_all_2, f"{config['log_directory']}/reference_cylinder_{config['deformation_bend']}{config['deformation_twist']}_mesh.pth".replace('.', '_'))
    ...
    spectral_coefficients = tch.einsum('ij, ki->jk', opt['evecs'].to(config['device']), v.to(config['device']))
    # spectral_coefficients
    
    latents = model.enc(spectral_coefficients[:opt['nb_freq']].unsqueeze(0).to(opt["device"]))
    # print(latents)
    ...
    scheduler = tch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, 
                                                         T_0=100,
                                                         T_mult=1,
    ...
    for iter in range(0, config['max_iters']):
        ...
        rmsd_orig = tch.sqrt(tch.mean((current_mesh_points - original_mesh_points) ** 2))
        ...
        chamfer_distance_overtime.append(chamfer_distance(reference_mesh_points.T.unsqueeze(0), current_mesh_points.T.unsqueeze(0)))
        
        # Calculate RMSD from previous iteration's mesh
        if iter > 0:
            rmsd_prev = tch.sqrt(tch.mean((current_mesh_points - previous_mesh_points) ** 2))
        ...
        _, tet_volumes, _ = get_volume_mesh(femesh_all_2_split['points'][0], femesh_all_2_split['elements'][0])
        ...
        try:
            ...
            print("Error in forward eigen decomposition (Non-hemritian encountered), stopping optimization here:", str(e))
        ...
        dif1 = tch.abs(tch.abs(tch.abs(mf_signal['signal_allcmpts']) / tch.abs(mf_signal['signal_allcmpts'][0, :, 0].view(1, -1, 1))) * 100) - (tch.abs(mf_signal_orig['signal_allcmpts']))
        ...
        print(f"Iteration {iter}: Loss = {loss.item()}, Learning Rate = {scheduler.get_last_lr()[0]}, Time taken = {time.time() - time_begin}")
    
        # Log parameter updates
        with tch.no_grad():
            if update_mask.grad is not None:
                param_update_norm = tch.norm(update_mask.grad * scheduler.get_last_lr()[0])
    ...
    tch.save(spectral_rmsd_from_original[:full_range], f"{config['log_directory']}/spectral_rmsd_from_original_overtime.pth")
    tch.save(spectral_rmsd_from_previous[:full_range], f"{config['log_directory']}/spectral_rmsd_from_previous_overtime.pth")
    tch.save(rmsd_from_reference[:full_range], f"{config['log_directory']}/rmsd_from_reference_overtime.pth")
    tch.save(rmsd_from_previous[:full_range], f"{config['log_directory']}/rmsd_from_previous_overtime.pth")
    tch.save(spectral_rmsd_from_original[:full_range], f"{config['log_directory']}/spectral_rmsd_from_original_overtime.pth")
    tch.save(loss_overtime[:full_range], f"{config['log_directory']}/loss_overtime.pth")
    tch.save(volumes_history, f"{config['log_directory']}/volumes_history.pth")
    tch.save(meshes_overtime, f"{config['log_directory']}/meshes_history.pth")
    tch.save(chamfer_distance_overtime, f"{config['log_directory']}/chamfer_distance_overtime.pth")
    ...
    def update(frame):
        ...
        if frame > 0:
            latent_dim = latent_points_history_np.shape[1]
            if latent_dim == 2:
                scatter = ax10.scatter(latent_points_history_np[:frame+1, 0], 
                                    latent_points_history_np[:frame+1, 1], 
                                    np.zeros(frame+1),
                                    c=range(frame+1), cmap='viridis')
                ...
                if n_components < 3:
                    ax10.set_title(f'Latent Space Trajectory (PCA)\nNot enough data points yet (n={frame+1})')
                    scatter = ax10.scatter([], [], [], c=[], cmap='viridis')
                else:
                    pca = PCA(n_components=n_components)
                    latent_points_pca = pca.fit_transform(latent_points_history_np[:frame+1])
                    if n_components == 3:
                        scatter = ax10.scatter(latent_points_pca[:, 0], 
                                            latent_points_pca[:, 1], 
                                            latent_points_pca[:, 2], 
                                            c=range(frame+1), cmap='viridis')
                    else:
    ...
    def create_video_with_opencv(update_func, num_frames, output_filename, fps=10):
        ...
        video = cv2.VideoWriter(output_filename, fourcc, fps, (frame.shape[1], frame.shape[0]))
    ...
    create_video_with_opencv(update, len(iterations_np), 
                            f"{config['experiment_name']}_optimization_progress.mp4", fps=5)

    # Display the final plot
    # plt.show()
    def create_video_from_two_folders(input_folder1, input_folder2, ref_image1_path, ref_image2_path, output_video, fps=5):
        # Get list of PNG files in the input folders
        png_files1 = natsorted([f for f in ops.listdir(input_folder1) if f.endswith('.png')])
        png_files2 = natsorted([f for f in ops.listdir(input_folder2) if f.endswith('.png')])
        ...
        ref_image1 = cv2.resize(ref_image1, (int(ref_image1.shape[1] * first_image1.shape[0] / ref_image1.shape[0]), first_image1.shape[0]))
        ref_image2 = cv2.resize(ref_image2, (int(ref_image2.shape[1] * first_image2.shape[0] / ref_image2.shape[0]), first_image2.shape[0]))
        
        # Calculate dimensions for the combined frame
        frame_width = ref_image1.shape[1] + max(first_image1.shape[1], first_image2.shape[1])
        ...
        for png_file1, png_file2 in zip(png_files1, png_files2):
            ...
            frame[:image1.shape[0], ref_image1.shape[1]:ref_image1.shape[1]+image1.shape[1]] = image1
            frame[image1.shape[0]:, ref_image2.shape[1]:ref_image2.shape[1]+image2.shape[1]] = image2
    ...
    create_video_from_two_folders(f"{config['experiment_name']}_p", 
                                    ...
                                    f"reference_cylinder_{config['deformation_bend']}{config['deformation_twist']}_p".replace('.', '_') + "/femesh_plot_iter_0.png", 
                                    f"reference_cylinder_{config['deformation_bend']}{config['deformation_twist']}_pc_p".replace('.', '_') + "/femesh_plot_iter_0.png", 
