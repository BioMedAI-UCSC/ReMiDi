...
def main(config):
    ...
    # 3D Mesh
    plot_femesh_plotly_2(rotated_mesh, setup.pde['compartments'], 0, f"reference_cylinder_bamp{config['beading_amplitude']}_bnp{config['beading_num_periods']}_r{config['rotation_inx']}_h".replace('.', '_'), f"reference_cylinder_bamp{config['beading_amplitude']}_bnp{config['beading_num_periods']}_r{config['rotation_inx']}_p".replace('.', '_'))
    # Point cloud
    plot_point_cloud_plotly(rotated_mesh['points'], 0, f"reference_cylinder_bamp{config['beading_amplitude']}_bnp{config['beading_num_periods']}_r{config['rotation_inx']}_pc_h".replace('.', '_'), f"reference_cylinder_bamp{config['beading_amplitude']}_bnp{config['beading_num_periods']}_r{config['rotation_inx']}_pc_p".replace('.', '_'))
    
    # Create logging directory
    ops.makedirs(config['log_directory'], exist_ok=True)
    
    # Saving complete reference mesh file
    tch.save(rotated_mesh, f"{config['log_directory']}/reference_cylinder_bamp{config['beading_amplitude']}_bnp{config['beading_num_periods']}_r{config['rotation_inx']}_mesh.pth".replace('.', '_'))
    ...
    create_video_from_two_folders(f"{config['experiment_name']}_p", 
                                    ...
                                    f"reference_cylinder_bamp{config['beading_amplitude']}_bnp{config['beading_num_periods']}_r{config['rotation_inx']}_p".replace('.', '_') + "/femesh_plot_iter_0.png", 
                                    f"reference_cylinder_bamp{config['beading_amplitude']}_bnp{config['beading_num_periods']}_r{config['rotation_inx']}_pc_p".replace('.', '_') + "/femesh_plot_iter_0.png", 
