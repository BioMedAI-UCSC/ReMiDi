...
def main(config):
    ...
    femesh_all_2["points"] = deform_domain_periodic_beading(
        femesh_all_2["points"],
        amplitude=config["beading_amplitude"],
        num_periods=config["beading_num_periods"],
    )
    ...
    plot_femesh_plotly_2(
        ...
        0,
        f"reference_cylinder_bamp{config['beading_amplitude']}_bnp{config['beading_num_periods']}_r{config['rotation_inx']}_h".replace(
            ".", "_"
        ),
        f"reference_cylinder_bamp{config['beading_amplitude']}_bnp{config['beading_num_periods']}_r{config['rotation_inx']}_p".replace(
            ".", "_"
        ),
    )
    # Point cloud
    plot_point_cloud_plotly(
        rotated_mesh["points"],
        0,
        f"reference_cylinder_bamp{config['beading_amplitude']}_bnp{config['beading_num_periods']}_r{config['rotation_inx']}_pc_h".replace(
            ".", "_"
        ),
        f"reference_cylinder_bamp{config['beading_amplitude']}_bnp{config['beading_num_periods']}_r{config['rotation_inx']}_pc_p".replace(
            ".", "_"
        ),
    )
    ...
    tch.save(
        rotated_mesh,
        f"{config['log_directory']}/reference_cylinder_bamp{config['beading_amplitude']}_bnp{config['beading_num_periods']}_r{config['rotation_inx']}_mesh.pth".replace(
            ".", "_"
        ),
    )
